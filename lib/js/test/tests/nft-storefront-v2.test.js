...
const setup_nft_account_tx = fs.readFileSync(path.resolve(__dirname, "../../mocks/transactions/setup_nft_account.cdc"), {encoding:'utf8', flag:'r'});
const setup_nba_nft_account_tx = fs.readFileSync(path.resolve(__dirname, "../../mocks/transactions/setup_nba_nft_account.cdc"), {encoding:'utf8', flag:'r'});
const mint_nft_tx = fs.readFileSync(path.resolve(__dirname, "../../mocks/transactions/mint_nft.cdc"), {encoding:'utf8', flag:'r'});
const get_owned_nft_ids_script = fs.readFileSync(path.resolve(__dirname, "../../mocks/scripts/get_owned_nft_ids.cdc"), {encoding:'utf8', flag:'r'});
const setup_account_to_receive_royalty_tx = fs.readFileSync(path.resolve(__dirname, "../../mocks/transactions/setup_account_to_receive_royalty.cdc"), {encoding:'utf8', flag:'r'});
const setup_account_to_receive_royalty_in_duc_tx = fs.readFileSync(path.resolve(__dirname, "../../mocks/transactions/setup_account_to_receive_royalty_in_duc_tx.cdc"), {encoding:'utf8', flag:'r'});
const setup_duc_receival_account_tx = fs.readFileSync(path.resolve(__dirname, "../../mocks/transactions/setup_duc_receival_account_tx.cdc"), {encoding:'utf8', flag:'r'});
const setup_duc_account_tx = fs.readFileSync(path.resolve(__dirname, "../../mocks/transactions/setup_duc_account_tx.cdc"), {encoding:'utf8', flag:'r'});
const sell_item_using_duc_tx = fs.readFileSync(path.resolve(__dirname, "../../../../transactions/dapper/sell_item_using_duc.cdc"), {encoding:'utf8', flag:'r'});
const buy_item_using_duc_tx = fs.readFileSync(path.resolve(__dirname, "../../../../transactions/dapper/buy_item_using_duc.cdc"), {encoding:'utf8', flag:'r'});
...
async function getListingKeys(account) {
  const read_storefront_ids_script = fs.readFileSync(path.resolve(__dirname, "../../../../scripts/read_storefront_ids.cdc"), {encoding:'utf8', flag:'r'});
  const [scriptResult, sError] = await executeScript({ code: read_storefront_ids_script, args: [account] });
  return scriptResult
}

async function getDuplicateListingIDs(account, nftId, listingId) {
  const read_duplicate_listing_ids_script = fs.readFileSync(path.resolve(__dirname, "../../../../scripts/read_duplicate_listing_ids.cdc"), {encoding:'utf8', flag:'r'});
  const [scriptResult, sError] = await executeScript({ code: read_duplicate_listing_ids_script, args: [account, nftId, listingId] });
  return scriptResult
}

async function getListingDetails(account, resourceId) {
  const read_listing_details_script = fs.readFileSync(path.resolve(__dirname, "../../../../scripts/read_listing_details.cdc"), {encoding:'utf8', flag:'r'});
  const [scriptResult, sError] = await executeScript({ code: read_listing_details_script, args: [account, resourceId] });
  return scriptResult
}
...
describe("NFTStorefrontV2", ()=> {
  ...
  let ducContractAddress;
  let nbaNFTContractAddress;
  ...
  let tokenForwardingContractAddress;
  ...
  beforeEach(async () => {
    ...
    nftStorefrontContractAddress = await getAccountAddress("nftStoreFrontV2");
    ...
    nonFungibleTokenContractAddress = await getAccountAddress("nonFungibleToken");
    // Deployed at address which has the alias - exampleNFT
    exampleNFTContractAddress = await getAccountAddress("exampleNFT");
    // Deployed at address which has the alias - dapperUtilityCoin
    ducContractAddress = await getAccountAddress("dapperUtilityCoin");
    // Deployed at address which has the alias - tokenForwarding
    tokenForwardingContractAddress = await getAccountAddress("tokenForwarding");
    // Deployed at address which has the alias - nbaNFT
    nbaNFTContractAddress = await getAccountAddress("nbaNFT");

    await deployContract({ to: fungibleTokenContractAddress,    name: "utility/FungibleToken"});
    await deployContract({ to: nonFungibleTokenContractAddress, name: "utility/NonFungibleToken"});
    await deployContract({ to: metadataViewsContractAddress,    name: "utility/MetadataViews"});
    await deployContract({ to: nftStorefrontContractAddress,    name: "NFTStorefrontV2"});
    await deployContract({ to: exampleNFTContractAddress,       name: "utility/ExampleNFT"});
    await deployContract({ to: ducContractAddress,              name: "utility/dapper/DapperUtilityCoin"});
    await deployContract({ to: tokenForwardingContractAddress,  name: "utility/dapper/TokenForwarding"});
    await deployContract({ to: nbaNFTContractAddress,           name: "utility/dapper/nba_NFT"});
  ...
  test("should successfully list the sale item", async() => {
    ...
    await shallPass(
      sendTransaction({
        code: mint_nft_tx,
        args: [seller_1, "NFT1", "This is to sell", "abc.jpeg", [], [], []],
    ...
    console.log(`Error for the transaction that need to reverted - ${error}`);
  ...
  test("should be able to sell an item and remove all existing listings", async() => {
      // step 1 : Setup a collection for seller_1
      await shallPass(
          sendTransaction({
              code: setup_nft_account_tx,
      ...
      var currentTimestamp = parseInt(await getCurrentTimestamp());
      ...
      const [txResult_2, error_2] = await shallPass(
          sendTransaction({
              name: "sell_item_and_replace_current_listing",
              args: [0, "50.50", "TopShot Platform", "10.50", currentTimestamp + 500, [marketplace_1, marketplace_2]],
              signers: [seller_1]
          })
      );
      ...
      // Validate the details
      var listingKeys = await getListingKeys(seller_1);
      expect(listingKeys.length).toEqual(1);

      var listingDetails = await getListingDetails(seller_1, listingKeys[0]);
      expect(listingDetails.purchased).toBeFalsy();
      expect(parseInt(listingDetails.expiry)).toEqual(currentTimestamp + 500);
      expect(listingDetails.customID).toEqual("TopShot Platform");
      expect(listingDetails.salePrice).toEqual('50.50000000');

      // Step 5: Make a new listing that should replace the current one.
      currentTimestamp = parseInt(await getCurrentTimestamp());

      const [txResult_3, error_3] = await shallPass(
          sendTransaction({
              name: "sell_item_and_replace_current_listing",
              args: [0, "60.50", "TopShot Platform", "10.50", currentTimestamp + 500, [marketplace_1, marketplace_2]],
              signers: [seller_1]
          })
      );

      // Make sure transaction did go through.
      expect(error_3).toEqual(null);

      // Validate the details
      listingKeys = await getListingKeys(seller_1);
      expect(listingKeys.length).toEqual(1);

      listingDetails = await getListingDetails(seller_1, listingKeys[0]);
      expect(listingDetails.purchased).toBeFalsy();
      expect(parseInt(listingDetails.expiry)).toEqual(currentTimestamp + 500);
      expect(listingDetails.customID).toEqual("TopShot Platform");
      expect(listingDetails.salePrice).toEqual('60.50000000');
  })
  ...
  test("should successfully list the sale item using sell_item_using_duc.cdc", async() => {
    // step 1 : Setup a collection for seller_1
    await shallPass(
      sendTransaction({
        code: setup_nft_account_tx,
    ...
    await shallPass(
      sendTransaction({
        code: setup_account_to_receive_royalty_in_duc_tx,
        args:[],
        signers: [artist_1]
      })
    );
    ...
    // Setup duc account for seller 1
    await shallPass(
      sendTransaction({
        code: setup_duc_account_tx,
        args:[],
    ...
    // Step 4.a: Should fail because marketplaces don't have valid DUC receiver type.
    const [txResult_2, error_2] = await shallRevert(
      sendTransaction({
        code: sell_item_using_duc_tx,
        args: [0, "50.50", "TopShot Platform", "10.50", currentTimestamp + 500, [marketplace_1, marketplace_2], ducContractAddress],
        signers: [seller_1]
      })
    ); 

    // Make sure transaction did not go through.
    expect(txResult_2).toEqual(null);
    ...
    await shallPass(
      sendTransaction({
        code: setup_duc_receival_account_tx,
        args:[],
        signers: [marketplace_1]
      })
    );
    ...
    // Successfully buy the same listing.
    const [txResult_purchase_3, error_purchase_3] = await shallPass(
      sendTransaction({
        code: buy_item_using_duc_tx,
        args: [listingKeys[0], seller_1, marketplace_1],
        signers: [buyer_1, ducContractAddress]
      })
    );
  ...
  //   const [marketplace_1_flow_balance_before, ] = await getFlowBalance(marketplace_1);
  //   const [marketplace_2_flow_balance_before, ] = await getFlowBalance(marketplace_2);
  ...
  //     sendTransaction({
  //       name: "sell_item",
  //       args: [0, "50.50", "TopShot Platform Duplicate List", "12.50", currentTimestamp + 800, [await getAccountAddress("marketplace_3"), await getAccountAddress("marketplace_4")]],
  //       signers: [seller_1]
  //     })
  //   );
  ...
  //   let duplicateListing = await getDuplicateListingIDs(seller_1, listingDetails.nftID, listingKeys[0]);
  ...
  //     sendTransaction({
  //       name: "buy_item",
  //       args: [listingKeys[0], seller_1, seller_2],
  //       signers: [buyer_1]
  //     })
  //   );
  ...
  //   console.log(`Error during first attempt of purchase - ${error_purchase}`);
  ...
  //   console.log(`Error during Second attempt of purchase - ${error_purchase_2}`);
  ...
  //   const [marketplace_1_flow_balance_after, ] = await getFlowBalance(marketplace_1);
  //   const [marketplace_2_flow_balance_after, ] = await getFlowBalance(marketplace_2);
  //   const [seller_1_balance_after, ] = await getFlowBalance(seller_1);

  //   expect(parseFloat(marketplace_1_flow_balance_after) - parseFloat(marketplace_1_flow_balance_before)).toEqual(parseFloat("10.50"));
  //   expect(parseFloat(marketplace_2_flow_balance_after) - parseFloat(marketplace_2_flow_balance_before)).toEqual(parseFloat("0.0"));  
  //   expect(parseFloat(artist_1_flow_balance_after) - parseFloat(artist_1_flow_balance_before)).toEqual(parseFloat("4.0"));
  //   expect(parseFloat(artist_2_flow_balance_after) - parseFloat(artist_2_flow_balance_before)).toEqual(parseFloat("10.0"));
  //   expect(parseFloat(seller_1_balance_after) - parseFloat(seller_1_balance_before)).toEqual(parseFloat("26.0"));
  ...
  //     sendTransaction({
  //       code: mint_nft_tx,
  //       args: [seller_1, "NFT2", "This is to sell as well", "xyz.jpeg", [], [], []],
  //       signers: [exampleNFTContractAddress]
  //     })
  //   );
  ...
  //     sendTransaction({
  //       code: mint_nft_tx,
  //       args: [seller_1, "NFT3", "This is to sell as well", "ama.jpeg", [], [], []],
  //       signers: [exampleNFTContractAddress]
  //     })
  //   );
  ...
  //     sendTransaction({
  //       name: "sell_item",
  //       args: [0, "50.50", "TopShot Platform 1", "10.50", currentTimestamp + 2,[]],
  //       signers: [seller_1]
  //     })
  //   ); 
  ...
  //     sendTransaction({
  //       name: "sell_item",
  //       args: [1, "50.50", "TopShot Platform 2", "10.50", currentTimestamp + 3,[]],
  //       signers: [seller_1]
  //     })
  //   ); 
  ...
  //     sendTransaction({
  //       name: "sell_item",
  //       args: [2, "50.50", "TopShot Platform 3", "10.50", currentTimestamp + 4,[]],
  //       signers: [seller_1]
  //     })
  //   ); 
  ...
  //     sendTransaction({
  //       name: "sell_item",
  //       args: [2, "50.50", "TopShot Platform 4", "10.50", currentTimestamp + 15,[]],
  //       signers: [seller_1]
  //     })
  //   ); 
  ...
  //   console.log(`Timestamp after the time jump - ${parseInt(await getCurrentTimestamp())}`);
